...
class Player {
    ...
    updateStats(bonus, isEquipping) {
        const modifier = isEquipping ? 1 : -1;
        if (bonus.strength)
            this.stats.strength += modifier * bonus.strength;
        if (bonus.mana)
            this.stats.mana += modifier * bonus.mana;
        if (bonus.agility)
            this.stats.agility += modifier * bonus.agility;
        if (bonus.life)
            this.stats.life += modifier * bonus.life;
    }
}
    ...
    create() {
        ...
        attackTypes.forEach((attack) => {
            this.add
                ...
                .on("pointerdown", () => {
                if (this.playerTurn)
                    this.handlePlayerAttack(attack.type);
            });
        });
        this.add
